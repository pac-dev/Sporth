...
int sporth_tanh(sporth_stack *stack, void *ud)
{
    plumber_data *pd = ud;
    SPFLOAT val;
    switch(pd->mode){
        case PLUMBER_CREATE:
            plumber_add_ugen(pd, SPORTH_TANH, NULL);
            if(sporth_check_args(stack, "f") != SPORTH_OK) {
                stack->error++;
                return PLUMBER_NOTOK;
            }
            val = sporth_stack_pop_float(stack);
            sporth_stack_push_float(stack, (SPFLOAT)tanh(val));
            break;
        case PLUMBER_INIT:
            val = sporth_stack_pop_float(stack);
            sporth_stack_push_float(stack, (SPFLOAT)tanh(val));
        ...
        case PLUMBER_COMPUTE:
            val = sporth_stack_pop_float(stack);
            sporth_stack_push_float(stack, tanh(val));
            break;
        case PLUMBER_DESTROY:
            break;
        default:
